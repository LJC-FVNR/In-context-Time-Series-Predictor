...
def random_sample_dataset(lst, num_elements=8):
...
class RandomizedDataLoaderIter:
    def __init__(self, dataloaders, sample_len=4096):
...
def data_provider(args, flag, element_wise_shuffle=True):
    if args.data_path in ['pretrain_config', 'baseline_config']:
        args.data_path = read_file_as_string(f"{args.data_path}.txt")
...
def data_provider_subset(args, flag):
    ...
    data_set = Data(
        ...
        do_forecasting=getattr(args, 'do_forecasting', False),
        min_max_scaling=getattr(args, 'min_max_scaling', False)
    )
